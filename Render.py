...
class Render(object):
    ...
    def load_model(self, model, scale_factorn =(1,1,1), translate_factor=(0,0,0),rotate=(0,0,0)):
        ...
        for face in cube.faces:
            ...
            if len(face) == 4:
                ...
                if self.texture and len(cube.vertices) !=0:
                    ft1 = face[0][1] - 1
                    ft2 = face[1][1] - 1
                    ft3 = face[2][1] - 1
                    ft4 = face[3][1] - 1
                    ...
                    if self.active_shader and len(cube.nvertices) !=0:
                        fn1 = face[0][2] - 1
                        fn2 = face[1][2] - 1
                        fn3 = face[2][2] - 1
    ...
    def shader(self,**kwargs):
        ...
        azuloscuro = color(48, 79, 109)
        azulosclaro = color(81,129,200)
        azumm = color(110,153,210)
        blancoazul = color(150,208,254)
        #print("Punto: " +str(x) + "  " + str(y))
        
        if y <= 782 and y >= 700 and x <= 720 and x >= 100:
            return color(self.clamping(48 * i), self.clamping(79 * i), self.clamping(109*i))
        if y <= 699 and y >= 650 and x <=720 and x >=100:
            return color(self.clamping(81 * i), self.clamping(129 * i), self.clamping(200*i))
        if y <= 649 and y >= 600 and x <=720 and x >=100:
            return color(self.clamping(110 * i), self.clamping(153 * i), self.clamping(210*i))
        if y <= 599 and y >= 550 and x <=720 and x >=100:
            return color(self.clamping(150 * i), self.clamping(208 * i), self.clamping(254*i))
        if y <= 549 and y >= 500 and x <=720 and x >=50:
            return color(self.clamping(110 * i), self.clamping(153 * i), self.clamping(210*i))
        if y <= 499 and y >= 450 and x <=720 and x >=50:
            return color(self.clamping(150 * i), self.clamping(208 * i), self.clamping(254*i))
        if y <= 449 and y >= 400 and x <=720 and x >=50:
            return color(self.clamping(110 * i), self.clamping(153 * i), self.clamping(210*i))
        if y <= 399 and y >= 350 and x <=720 and x >=50:
            return color(self.clamping(150 * i), self.clamping(208 * i), self.clamping(254*i))
        if y <= 349 and y >= 250 and x <=720 and x >=50:
            return color(self.clamping(48 * i), self.clamping(79 * i), self.clamping(109*i))
